...
import com.project.service.FamilyEventService;

@RestController
@RequestMapping("/Familyevents")
public class FamilyEventController {

	@Autowired
	private FamilyEventService eventService;

	@PostMapping("/create/{familyId}")
	public ResponseEntity<Object> createEventForFamily(@PathVariable Long familyId, @RequestBody FamilyEvent event) {
		try {
			ResponseEntity<List<FamilyEvent>> response = eventService.createEventForFamily(familyId, event);
	...
	public ResponseEntity<Object> getAllEvents() {
		try {
			List<FamilyEvent> events = eventService.getAllEvents();
	...
	public ResponseEntity<Object> getEventById(@PathVariable Long eventId) {
		try {
			ResponseEntity<FamilyEvent> response = eventService.getEventById(eventId);
	...
	public ResponseEntity<Object> updateEvent(@PathVariable Long eventId, @RequestBody FamilyEvent updatedEvent) {
		try {
			ResponseEntity<FamilyEvent> response = eventService.updateEvent(eventId, updatedEvent);
			if (response.getStatusCode() == HttpStatus.OK) {
				return ResponseEntity.ok(response.getBody());
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Event not found with eventId: " + eventId);
